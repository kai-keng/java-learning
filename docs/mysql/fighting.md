...
1. 有些系统中使用数据库是否只读来判断该库为主库还是从库，如果全局修改的话，可能会影响判断。
2. 当我们的客户端备份连接突然异常中断的时候，如果使用 FTWRL 命令，中断以后会自动解锁，让业务恢复正常，但是使用设置只读的方式库依旧会保持只读，继续影响业务。
...
> 生产加字段、修改字段和添加索引操作需要全表扫描，若生产上的大表修改会阻塞很长时间，需慎重。小表也需要慎重。
