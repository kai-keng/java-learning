...
### 配合全局异常处理优化返回结果
在参数验证错误的时候，会抛出一些异常，并且返回结果，但是这个返回结果针对于业务来说并不是那么的友好，在业务处理中，我们一般都会规范的我们返回结果，统一样式返回。我们可以配合全局异常管理，全局捕捉参数异常，然后统一返回，详细可以参照上面的文章[统一异常处理](http://localhost:8080/java-learning/spring-boot/1-exception.html#%E5%89%8D%E8%A8%80)
